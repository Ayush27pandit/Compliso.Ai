...
### Eval philosophy
Every prompt/retrieval change is checked against a regression suite before it ships — CI-gated, not vibes-gated. Core test categories:
- **Conflict resolution** — does the system resolve to the correct, current figure when sources disagree?
- **Speculation rejection** — does it avoid stating rumored/unconfirmed changes as fact?
- **Distractor rejection** — does retrieval stay on-topic under noise?
- **Gap honesty** — when a source is illegible/incomplete, does the system say so instead of hallucinating a number?

---

## Embedding Model Selection

Selected via a domain-specific eval set (not by trusting the MTEB leaderboard blindly — see rationale below), benchmarked on retrieval recall@10 / nDCG@10 against real query→chunk pairs from `true_data/`.

| Candidate | Why considered |
|---|---|
| **BGE-M3** *(current default)* | MIT license, self-hostable, native multilingual (covers Hindi-English mixed regulatory terminology), hybrid dense+sparse+multi-vector retrieval in one model |
| Qwen3-Embedding | Apache-2.0, strong multilingual retrieval ceiling, used as a benchmark comparison |
| API-based alternatives (OpenAI / Gemini) | Evaluated for zero-infra scenarios; not selected due to self-hosting preference and data-sovereignty considerations for regulatory content |

Full methodology: MTEB retrieval/STS sub-scores used only as an initial shortlist filter; final selection based on in-domain recall against a hand-labeled eval set, plus license, dimensionality/cost, and latency constraints.
...
- **Queue**: BullMQ (async job orchestration for ingestion + long-running retrieval evals)
- **Embeddings / Vector Store**: BGE-M3 + [vector DB — pgvector / Qdrant]
- **LLM**: Claude API (Anthropic)
- **Guardrails**: Custom middleware — input/output validation, conflict detection, source-authority weighting
...
```bash
git clone https://github.com/<your-org>/compliso-ai.git
cd compliso-ai
npm install
cp .env.example .env   # add DB, Redis, and Anthropic API credentials
npx prisma migrate dev
npm run ingest         # embeds true_data/ and noisy_data/ into the vector store
npm run eval           # runs the regression suite before you touch prompts
npm run dev
```
...
- [ ] Expand `true_data/` coverage: TDS/TCS filing, professional tax, labour law basics for MSMEs
- [ ] Hindi-language query support (native, not translated)
- [ ] Durable execution for multi-step compliance workflows (e.g., "help me register and file my first GST return")
- [ ] Canary rollout for prompt/embedding model changes with automatic rollback on eval regression
- [ ] Public eval leaderboard for transparency on answer accuracy over time
